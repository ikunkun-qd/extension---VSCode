...
export class SettingsViewProvider implements vscode.WebviewViewProvider {
    ...
    private async _saveConfiguration(config: any): Promise<void> {
        try {
            ...
            const normalizedActual = this.normalizePath(verifyBasePath as string);
    ...
    private async _previewMapping(componentName: string): Promise<void> {
        try {
...
<body>
     ...
     * 确定配置保存目标
     */
    private determineConfigTarget(config: any): vscode.ConfigurationTarget {
        // 检查是否有工作区
