...
class ConfigManager {
     ...
     * 合并不同来源的配置
     */
    mergeConfigurations(workspaceConfig) {
        // 1. 默认配置
        const defaultConfig = {
            basePath: '',
            mappingRule: {},
    ...
    getGlobalConfig() {
        try {
            ...
            const globalConfig = {
                basePath: globalWorkspaceConfig.inspect('basePath')?.globalValue || '',
                mappingRule: globalWorkspaceConfig.inspect('mappingRule')?.globalValue || {},
                cacheTimeout: globalWorkspaceConfig.inspect('cacheTimeout')?.globalValue || 300000
            };
     ...
     * @returns 匹配的文档路径
     */
    findFuzzyMatch(componentName, basePath) {
        const fs = require('fs');
        const path = require('path');
     ...
     * @returns 匹配模式数组
     */
    generateMatchPatterns(componentName) {
        const patterns = [];
